...
impl Transform {
    ...
    pub fn final_model<'a>(
        ...
        let final_scale =
            Affine3A::from_scale(self.final_scale(child, children, transforms, scale_cache));
        let final_model_without_scale =
            self.final_model_without_scale(child, children, transforms, model_cache);
    ...
    pub fn final_model_without_scale<'a>(
        &self,
        child: &Child,
        children: impl Get<Out = &'a Child> + Copy, // &View<Child> or &ViewMut<Child>
        transforms: impl Get<Out = &'a Transform> + Copy, // &View<Transform> or &ViewMut<Transform>
        ...
        if let Some(parent_final_model_without_scale) =
    ...
    pub fn final_scale<'a>(
        ...
        if let Some(parent_final_scale) =
    ...
    pub fn final_model_2d<'a>(
        ...
        let final_scale =
            Affine2::from_scale(self.final_scale_2d(child, children, transforms, scale_cache));
        let final_model_without_scale =
            self.final_model_without_scale_2d(child, children, transforms, model_cache);
        final_model_without_scale * final_scale
    }
    ...
    /// which is parent_final_model_without_scale * self.model_without_scale.
    pub fn final_model_without_scale_2d<'a>(
        &self,
        child: &Child,
        children: impl Get<Out = &'a Child> + Copy, // &View<Child> or &ViewMut<Child>
        transforms: impl Get<Out = &'a Transform> + Copy, // &View<Transform> or &ViewMut<Transform>
        ...
        if let Some(parent_final_model_without_scale) =
            Self::entity_final_model_without_scale_2d(child.parent(), children, transforms, cache)
        {
