...
use steel_common::{app::{App, Command, EditorCamera, WindowIndex}, data::{Value, WorldData}};
...
impl Editor {
    ...
    pub fn ui(&mut self, gui: &mut Gui, gui_game: &mut Option<Gui>,
        ...
        gui.immediate_ui(|gui| {
            ...
            self.menu_bar.ui(&mut self.editor_state, &mut self.scene_window, &mut self.game_window, &mut self.data_window, &ctx, gui, gui_game, &mut self.dock_state, context, project, world_data, local_data, &mut self.texts);
            ...
            if project.is_compiled() {
                ...
                if !self.editor_state.use_dock {
                    self.scene_window.show(&ctx, gui, context, renderer, &self.texts);
                    self.game_window.show(&ctx, gui, context, renderer, &self.texts);
                }
            ...
            if !self.editor_state.use_dock {
                if let Some(world_data) = world_data {
                    if let Some(app) = project.app() {
                        self.data_window.entity_component_windows(&ctx, world_data, app, &self.texts);
                        self.data_window.unique_windows(&ctx, world_data, &self.texts);
                    }
                }
            }
            ...
            if self.editor_state.use_dock {
                DockArea::new(&mut self.dock_state)
                    ...
                    .show(&ctx, &mut MyTabViewer { texts: &self.texts, add_contents: Box::new(|ui, tab| {
                        match tab.as_str() {
                            ...
                            "Entities" => if let Some(world_data) = world_data {
                                if let Some(app) = project.app() {
                                    self.data_window.entities_view(ui, world_data, app, &self.texts);
                                }
                            },
                            "Entity" => if let Some(world_data) = world_data {
                                if let Some(app) = project.app() {
                                    if let Some(entity_data) = world_data.entities.get_mut(&self.data_window.selected_entity()) {
                            ...
                            "Unique" => if let Some(world_data) = world_data {
                                if let Some(unique_data) = world_data.uniques.get_mut(self.data_window.selected_unique()) {
                                    DataWindow::data_view(ui, self.data_window.selected_unique(), unique_data);
                                }
                            },
                            _ => (),
                        }
                ...
                self.editor_state.focused_tab = self.dock_state.find_active_focused().map(|(_, tab)| tab.clone());
            }
        });
    }
    ...
    fn drag_entity(&mut self, world_data: &mut WorldData, input: &WinitInputHelper, editor_camera: &EditorCamera) {
        if input.mouse_held(0) && self.data_window.selected_entity() == self.editor_state.pressed_entity {
            if let Some(entity_data) = world_data.entities.get_mut(&self.data_window.selected_entity()) {
    ...
    fn scene_focus(&self) -> bool {
        if self.editor_state.use_dock {
            self.editor_state.focused_tab.as_ref().is_some_and(|tab| tab == "Scene")
        } else {
    ...
    fn game_focus(&self) -> bool {
        if self.editor_state.use_dock {
            self.editor_state.focused_tab.as_ref().is_some_and(|tab| tab == "Game")
        } else {
