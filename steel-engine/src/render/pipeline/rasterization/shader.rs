/// The shader to draw vertices. Used to draw points and lines.
pub mod vertex {
    use glam::{Vec3, Vec4};
...
/// Use [super::mesh::VertexData] and [super::mesh::InstanceData].
pub mod circle {
    pub mod vs {
        vulkano_shaders::shader! {
            ty: "vertex",
            src: r"
                ...
                layout(location = 3) in uvec2 eid;
                layout(location = 4) in uint index;
                layout(location = 5) in mat4 model; // position is center, scale is radius

                layout(location = 0) out vec2 out_position;
                layout(location = 1) out vec2 out_tex_coord;
                layout(location = 2) out vec4 out_color;
                layout(location = 3) out uvec2 out_eid;
    ...
    pub mod fs {
        vulkano_shaders::shader! {
            ...
            src: r"
                ...
                layout(location = 3) flat in uvec2 in_eid;
                layout(location = 4) flat in uint i;
