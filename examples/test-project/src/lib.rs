...
impl Engine for EngineImpl {
    ...
    fn draw(&mut self, info: DrawInfo) -> Box<dyn GpuFuture> {
        self.world.add_unique(RenderInfo { device: info.context.device().clone(), graphics_queue: info.context.graphics_queue().clone(),
            memory_allocator: info.context.memory_allocator().clone(), window_size: info.window_size, image: info.image,
...
fn render2d_system(info: UniqueView<RenderInfo>, transform2d: View<Transform2D>, renderer2d: View<Renderer2D>) -> PrimaryAutoCommandBuffer {
