...
pub fn camera_maintain_system(mut transform2d: ViewMut<Transform2D>, camera: View<Camera>, mut info: UniqueViewMut<CameraInfo>) {
    if let Some((e, camera)) = camera.iter().with_id().next() {
        ...
        info.position = transform2d.position;
        // TODO: transform2d.rotation
